...
<select id="item_#{attribute}" name="item[#{attribute}]">
  ...
  def typus_collection_field(attribute, value)
    returning(String.new) do |html|
      ...
      html << <<-HTML
<li><label for="item_#{attribute}">#{attribute.titleize.capitalize} <small>#{link_to "Add new", "/admin/#{attribute.titleize.tableize}/new?back_to=#{request.env['REQUEST_URI']}", :confirm => "Are you sure you want to leave this page?\nAny unsaved data will be lost." }</small></label>

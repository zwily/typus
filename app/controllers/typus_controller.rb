class TypusController < ApplicationController
  ...
  #
  def dashboard
    flash[:notice] = "There are not defined applications in config/typus/*.yml" if Typus.applications.empty?
  end
  ...
  def setup
    ...
    if request.post?
      ...
      if @typus_user.save
        ...
        flash[:notice] = "Your new password is <strong>%s</strong>." % password

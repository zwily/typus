...
You can configure all *Typus* settings from the <tt>application.yml</tt> 
file located under the <tt>config/typus</tt> folder. This file is created 
once you run the <tt>typus</tt> generator.
...
NOTE: Upload files only works if you follow <tt>Paperclip</tt> naming 
conventions.
...
You can define search filters on <tt>config/typus/application.yml</tt>.

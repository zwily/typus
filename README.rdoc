...
Install from GitHub the latest version which works with Rails 2.3.2.

    $ script/plugin install git://github.com/fesplugas/typus.git

Once the plugin is installed generate the *Typus* files and migrate 
your database, only a <tt>typus_users</tt> table is created.

    $ script/generate typus
    $ rake db:migrate

To create the first user, to do it, start the application server, go 
to http://0.0.0.0:3000/admin and follow the instructions.
...
If you need to use *Typus* with Rails 2.2.2 install the 2.2.2 branch 
of the plugin. This version will be mantained during 3 months from the 
...
from the yaml files under <tt>config/typus</tt>.
...
By default Typus is in English but you can change this with the 
...
This will make appear on the right bottom of the site a languages list.
...
There are two kind of files, the <tt>*.yml</tt> and the <tt>*_roles.yml</tt>:
    ...
    ##
    # Default appearence of application.yml
    #

    ##
    # Default application of application_roles.yml
    #
...
Upload files only works if you follow <tt>Paperclip</tt> naming conventions.
...
To define <tt>auto generated</tt> fields.
...
You can add <tt>slug</tt> a as attribute and it'll be shown on the lists.
...
You can define which filters you want to have on an specific model. 
...
You can set the default display order of the items on a model.
...
You can define which fields will be used when performing a search on 
the model.
...
You can define more actions which will be shown on the requested action.
...
You need to have <tt>mocha</tt> and <tt>redgreen</tt> to test the plugin.

    $ sudo gem install mocha redgreen

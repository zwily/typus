class TypusPluginGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      m.file "config/typus.yml.template", "vendor/plugins/#{file_name}/config/typus.yml"
      m.file "config/typus_roles.yml.template", "vendor/plugins/#{file_name}/config/typus_roles.yml"
    end
  end
